...
The returned Expression is as similar as possible to the code generated by Roslyn for the `foreach` keyword. It depends on:

- The enumerator is an `interface`, `class`, `struct`, or `ref struct`.
- The enumerator disposable or not.
- If the enumerable is an array, it uses the array indexer instead of `IEnumerable<>` to enumerate.

Throws an exception if the Expression in the first parameter does not define an enumerable. In case you don't want the expcetion to be thrown, use the other overload that takes an `EnumerableInfo` or `EnumerableSymbols` for the first parameter. Use `IsEnumerable` to get the required values.
...
The returned Expression is as similar as possible to the code generated by Roslyn for the `using` keyword.
